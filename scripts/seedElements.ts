...
async function seed() {
  console.log("Connecting to E118 Repository...");
  const productsRef = collection(db, "products");

  // Clear existing
  console.log("Purging old inventory...");
  ...
  // Seed new
  console.log("Seeding all 118 atomic products...");
  for (const el of elements) {
    ...
    try {
      await setDoc(doc(db, "products", `el-${el.atomic}`), product);
  ...
  console.log("E118 CATALOG EXPANSION SUCCESSFUL.");
  process.exit(0);
}
